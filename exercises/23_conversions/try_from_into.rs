...
fn is_valid_int(i: i16) -> bool {
    (0..=255).contains(&i)
}

fn convert_to_u8(i: i16) -> u8 {
    i as u8
}
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if !is_valid_int(tuple.0) || !is_valid_int(tuple.1) || !is_valid_int(tuple.2){
            return Err(IntoColorError::IntConversion);
        }

        Ok(Color { red: convert_to_u8(tuple.0), green: convert_to_u8(tuple.1), blue: convert_to_u8(tuple.2) })
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if !is_valid_int(arr[0]) || !is_valid_int(arr[1]) || !is_valid_int(arr[2]){
            return Err(IntoColorError::IntConversion);
        }

        Ok(Color { red: convert_to_u8(arr[0]), green: convert_to_u8(arr[1]), blue: convert_to_u8(arr[2]) })
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        Ok(Color { red: convert_to_u8(*slice.first().unwrap()), green: convert_to_u8(*slice.get(1).unwrap()), blue: convert_to_u8(*slice.get(2).unwrap()) })
    }
}
